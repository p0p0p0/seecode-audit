...
class Paginator(object):
    def __init__(self, object_list, per_page, orphans=0, allow_empty_first_page=True, request=None, pre_name=''):
    ...
    def page(self, number):
        ...
        if top + self.orphans >= self.count:
            top = self.count
        return Page(self.object_list[bottom:top], number, self, pre_name=self.pre_name)
...
class Page(object):
    def __init__(self, object_list, number, paginator, pre_name=''):
        self.object_list = object_list
    ...
    def _other_page_querystring(self, page_number):
        ...
        if self.paginator.request:
            self.base_queryset['p'] = page_number
